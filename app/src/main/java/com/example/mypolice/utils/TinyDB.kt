...
//        return objects
//    }
    fun getListObject(key: String?): ArrayList<ModelSaudara> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList: ArrayList<ModelSaudara> = ArrayList<ModelSaudara>()
        for (jObjString in objStrings) {
            val player: ModelSaudara = gson.fromJson(jObjString, ModelSaudara::class.java)
    ...
    fun putListObject(key: String?, objArray: ArrayList<ModelSaudara>) {
        checkForNullKey(key)
        val gson = Gson()
